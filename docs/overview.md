...
| Build type      | Default tag                          | Compiled backends and userspace                                      |
|:----------------|:-------------------------------------|:--------------------------------------------------------------------|
| `rocm`          | `localhost/strix-llama:rocm`         | HIP plus Vulkan/RADV; stable ROCm userspace (default 7.2.4).       |
| `rocm-next`     | `localhost/strix-llama:rocm-next`    | HIP plus Vulkan/RADV; separate TheRock nightly ROCm userspace.     |
| `vulkan`        | `localhost/strix-llama:vulkan`       | Vulkan/RADV only; ROCm-independent Fedora Mesa userspace.          |
| `vulkan-fpx`    | `localhost/strix-llama:vulkan-fpx`   | Vulkan/RADV only ROCmFPX fork; ROCm-independent Mesa userspace.    |
| `rocm-fpx`      | `localhost/strix-llama:rocm-fpx`     | HIP plus Vulkan/RADV ROCmFPX fork; stable ROCm userspace.          |
| `rocm-next-fpx` | `localhost/strix-llama:rocm-next-fpx`| HIP plus Vulkan/RADV ROCmFPX fork; separate nightly userspace.     |
...
branch `main` and additionally retain the fork's `llama-completion` and
`llama-perplexity` utilities. Test executables are not runtime artifacts.
...
- Qwen3.6 preset defaults: `ctx-size = 262144` total server context pool,
  ...
  checkpoints (`cache-ram = 65536`), reasoning-on coding-agent sampling
  defaults;
- Qwen/Qwen-derived `~non-reasoning` routes require
  `--with-non-reasoning` (`reasoning = off`, non-thinking sampling); they are
  not generated by default, except Crown Halo, which always emits its two MTP
  reasoning/non-reasoning routes;
- vision routes require `--with-vision`; paired `mmproj*.gguf` routes then add
...
- backend-specific microbatch defaults: `512` for Vulkan/FPX, `2048` for stock
  ROCm;
- `/dev/dri` for Vulkan-only images, and `/dev/dri` plus `/dev/kfd` for the
  combined HIP/Vulkan ROCm images;
