...
Run the images directly with Podman through `bin/run.sh`, which mounts each
runtime family with the correct devices and the Strix Halo defaults.

## Backends

The helper script supports the local images:

| Backend | Image | Backends in image | GPU devices |
| :--- | :--- | :--- | :--- |
| `rocm` | `localhost/strix-llama:rocm` by default, or `:rocm-$CPU_TARGET` when `CPU_TARGET!=generic` | HIP + Vulkan/RADV (stable ROCm) | `/dev/dri`, `/dev/kfd` |
| `rocm-7.2.4` | `localhost/strix-llama:rocm-7.2.4` by default, or `:rocm-7.2.4-$CPU_TARGET` when `CPU_TARGET!=generic` | HIP + Vulkan/RADV (stable ROCm) | `/dev/dri`, `/dev/kfd` |
| `rocm-next` | `localhost/strix-llama:rocm-next` by default, or `:rocm-next-$CPU_TARGET` when `CPU_TARGET!=generic` | HIP + Vulkan/RADV (nightly ROCm) | `/dev/dri`, `/dev/kfd` |
| `rocm7-nightlies` | `localhost/strix-llama:rocm7-nightlies` by default, or `:rocm7-nightlies-$CPU_TARGET` when `CPU_TARGET!=generic` | HIP + Vulkan/RADV (nightly ROCm) | `/dev/dri`, `/dev/kfd` |
| `vulkan` | `localhost/strix-llama:vulkan` by default, or `:vulkan-$CPU_TARGET` when `CPU_TARGET!=generic` | Vulkan/RADV only; ROCm-independent | `/dev/dri` |
| `vulkan-fpx` | `localhost/strix-llama:vulkan-fpx` by default, or `:vulkan-fpx-$CPU_TARGET` when `CPU_TARGET!=generic` | Vulkan/RADV only; ROCm-independent | `/dev/dri` |
| `rocm-fpx` | `localhost/strix-llama:rocm-fpx` by default, or `:rocm-fpx-$CPU_TARGET` when `CPU_TARGET!=generic` | HIP + Vulkan/RADV (stable ROCm) | `/dev/dri`, `/dev/kfd` |
| `rocm-next-fpx` | `localhost/strix-llama:rocm-next-fpx` by default, or `:rocm-next-fpx-$CPU_TARGET` when `CPU_TARGET!=generic` | HIP + Vulkan/RADV (nightly ROCm) | `/dev/dri`, `/dev/kfd` |
...
Combined ROCm images can report the same physical APU through HIP and Vulkan.
The tested selectors are `ROCm0` for HIP and `Vulkan0` for RADV. Generated
presets default to `ROCm0` for ROCm images; override that choice with:

```bash
bin/run.sh --device Vulkan0 --with-configs rocm-fpx server
```

`LLAMA_DEVICE=Vulkan0` provides the equivalent environment-based override.
Standalone `vulkan` and `vulkan-fpx` images remain useful when ROCm userspace is
not wanted.
...
speculation settings. Qwen/Qwen-derived `~non-reasoning` routes require
`--with-non-reasoning`; they are not generated by default. The Crown Halo
special case is documented below.

The generated Qwen3.6 presets use `ctx-size = 262144` as the total server
context pool, `parallel = 1`, f16 KV cache, device KV offload, unified KV,
`ctx-checkpoints = 32`, `checkpoint-min-step = 256`, prompt caching with
`cache-ram = 65536`, `reasoning = on`, and provider-qualified model names. The
single slot gets the full context pool
...
Vulkan. Vision routes require `--with-vision`; only those paired-mmproj routes
set `image-min-tokens = 1024`. `parallel > 1` splits `ctx-size` across server
...
`--checkpoint-every-n-tokens` flag. The checkpoint defaults avoid repeated full
prompt prefill after follow-up turns where a checkpoint can be restored.
...
(`Vulkan0` and `ROCm0` are illustrative names; confirm names with
`llama-cli --list-devices` for the selected image); ROCm device sections emit
...
podman run --rm -it \
  ...
  llama-server --models-preset /tmp/llama-models.ini --models-max 1 \
    ...
    --batch-size 2048 --ubatch-size 512 -fa 1 --no-mmap
```
...
podman run --rm -it \
  ...
  llama-server --models-preset /tmp/llama-models.ini --models-max 1 \
    --host 0.0.0.0 --port 8080 \
    --batch-size 4096 --ubatch-size 2048 -fa 1 --no-mmap
```
...
The helper defaults `server`, `mtp-server`, `load-test`, and `cli` to `-fa 1` and `--no-mmap` because those are required for reliable Strix Halo runs. Preset `server` applies those defaults on the command line even with an explicit `LLAMA_MODELS_PRESET`; an explicit command-line flash-attention or mmap option remains a user override. Generated presets omit stock Qwen `~non-reasoning` variants, ROCmFPX `~non-reasoning` variants, and mmproj-backed `~vision` variants unless `bin/run.sh` is called with `--with-non-reasoning` or `--with-vision`; Crown Halo always emits its two MTP reasoning/non-reasoning routes. For `bench`, it uses `-fa 1`, `-mmp 0`, `-p 2048`, `-n 32`, `-d 131072`, and the backend-specific `-ub` value.
